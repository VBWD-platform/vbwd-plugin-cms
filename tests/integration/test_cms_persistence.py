...
class TestCmsPagePersistence:
    ...
    def test_create_then_update_page(self, client, db, auth_headers):
        ...
        """
        # Create
        resp = client.post("/api/v1/admin/cms/pages", json={
        ...
        resp = client.put(f"/api/v1/admin/cms/pages/{page_id}", json={
            "name": "Persistence Test Updated",
        }, headers=auth_headers)
        assert resp.status_code == 200, f"PUT returned {resp.status_code}: {resp.get_json()}"
...
class TestCmsCategoryPersistence:
    def test_create_then_update_category(self, client, db, auth_headers):
        resp = client.post("/api/v1/admin/cms/categories", json={
            "name": "Test Cat",
        ...
        resp = client.put(f"/api/v1/admin/cms/categories/{cat_id}", json={
    ...
    def test_create_then_delete_category(self, client, db, auth_headers):
        ...
        resp = client.delete(f"/api/v1/admin/cms/categories/{cat_id}", headers=auth_headers)
        assert resp.status_code == 204
...
class TestCmsBulkPersistence:
    ...
    def test_bulk_publish_persists(self, client, db, auth_headers):
        ...
        resp = client.post("/api/v1/admin/cms/pages/bulk", json={
            "ids": ids,
            "action": "publish",
        }, headers=auth_headers)
        assert resp.status_code == 200
